...
function addPlaceFunction(req, res, reqBody) {
    ...
    fs.readFile('places.json', function(err, content) {
        ...
        var data = querystring.parse(reqBody);
        parseJson.placeList.push({
            ...
            "additionalinfourl": data.additionalinfourl
        });
        fs.writeFile('places.json', JSON.stringify(parseJson, null, "\t"), "UTF-8", function(err, parseJson) {
            if (err) {
                throw err;
            }
        });
